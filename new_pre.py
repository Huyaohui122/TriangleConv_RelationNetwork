...
def get_line_features_final(df_node, POINTS_SHAPE=20):
    ...
    df_line['dl'] = pd.Series([(dx ** 2 + dy ** 2) ** 0.5 for dx, dy in df_line[['dx', 'dy']].values])#勾股定理 算出两点之间的直线距离
    df_line['dr'] = df_line['dl'] / df_line['length']#算出两点间的距离占这条曲线的比例
    #print("This is df_length",df_line['dr'])
    #
    ...
    df_line['_dy'] = df_line['dy'] / df_line['_dr']#对应比例进行缩放
    #
    ...
    #print("This is more",df_line['more'])#对应的点需要位移的比例
